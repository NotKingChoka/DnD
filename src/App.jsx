...
const chestActions = [
  {
    ...
    failure: 'Ты не замечаешь подвоха. гла щёлкает, крышка остаётся закрытой.',
...
const guardActions = [
  ...
  },
  {
    ...
    failure: 'Стражник прищуривается. стория звучит подозрительно.',
    achievementId: 'silver-tongue',
  },
  {
    id: 'intimidate',
  ...
  return (
    <div className="app-shell">
      ...
      <main className="page-shell">
            ...
            <section className="panel trial-section" ref={trialsRef}>
              ...
              <div className="trial-grid">
                <MiniGameCard
                  ...
                  description="Перед тобой древний запертый сундук.  еши, как герой попробует справиться с ним."
        ...
        {progress.view === 'wiki' && (
          <Suspense
            fallback={<div className="panel wiki-fallback">  </div>}
          >
...
function LessonChecklist({ lessonState }) {
  const items = [
    { id: 'theory', title: 'Теория', note: 'Короткое объяснение темы' },
    { id: 'example', title: 'Пример', note: 'Живая анимационная сцена' },
    { id: 'practice', title: 'Практика', note: 'нтерактив и мини-проверка' },
    { id: 'miniGame', title: 'Мини-игра', note: 'Отдельное тематическое испытание' },
  ]
      ...
      <div className="lesson-grid">
        <div className="lesson-copy">
          <div className="lesson-heading lesson-section">
            <span className="section-kicker">Теория</span>
          ...
          <div className="lesson-actions">
            ...
            >
              {isCompleted
                ...
                : canFinish
                  ? 'Урок освоен'
                  : 'Закрой теорию, практику и мини-игру'}
            </button>
      ...
      <div className="practice-shell">
        <div className="practice-header">
          ...
          <span className={`result-pill ${practiceReady ? 'is-success' : ''}`}>
            {practiceReady ? 'Шаг зачтён' : 'Ждёт действия'}
          </span>
        </div>

        {practiceType === 'quiz' && (
          <div className="practice-outcome">
            <strong>Этот урок закрепляется через короткую мини-проверку ниже.</strong>
            <p>Выбери верный ответ, чтобы закрыть практический шаг урока.</p>
          </div>
        )}
        ...
        {practiceType === 'combat' && (
          <CombatDemo onVictory={() => markPracticeInteraction()} soundEnabled={soundEnabled} title="Тренировочный бой" />
        )}
      ...
      <div className={`scene-frame scene-${lesson.exampleType}`}>
        ...
        {lesson.exampleType === 'classes' && (
          <div className="scene-class-stack">
            {['Воин', 'Маг', ' азбойник', 'Бард'].map((item, index) => (
...
function CombatDemo({ onVictory, soundEnabled, title = 'Первый бой' }) {
  ...
  const startFight = () => {
    ...
    setState((previous) => ({
      ...previous,
      log: addLog(
        `нициатива: ты ${playerInitiative}, гоблин ${enemyInitiative}. ${
    ...
    <div className="exam-shell">
      ...
      {completed && (
        <div className={`quiz-feedback ${passed ? 'success' : 'fail'}`}>
           езультат: {score} / {FINAL_EXAM_QUESTIONS.length}.{' '}
          {passed
